...
public abstract class SplitStringUdx
{
     ...
     * causing split.
     * @param trimTokens trim leading and trailing whitespace from split tokens
     * (TRUE/FALSE)
        ...
        throws SQLException, ApplibException
    {
        ...
        if (inputSet.getMetaData().getColumnCount() != 1) {
            throw ApplibResource.instance().InputMustBeSingleColumn.ex();
        }
        ...
        throws SQLException, ApplibException
    {
        ...
        if (columnNameList.size() != 1) {
            throw ApplibResource.instance().SplitColNameMustBeSingleColumn.ex();
        }
     ...
     * @param startNum the starting sequence number or null if no sequence
            ...
            if (!tokens.hasNext()) {
                for (int colIndex = 1; colIndex <= columnCount; colIndex++) {
                    if (colIndex == stringColIndex) {
                        ...
                        resultInserter.setObject(
                            colIndex,
                            inputSet.getObject(colIndex));
                    }
                }
                if (startNum != null) {
        ...
        while (i <= theString.length()) {
                ...
                || (theString.charAt(i) == separator))
            {
                    ...
                    && (theString.charAt(i - 1) == escape))
                {
                    ...
                    theString =
                        theString.substring(0, i - 1) + theString.substring(i);
                    continue;
                }
            }
            i++;
     ...
     * causing split.
     *
     * @returns true if (and only if) both parameters are one character strings.
        ...
        throws ApplibException
    {
        ...
        if (separator.length() != 1) {
            throw ApplibResource.instance().SeparatorMustBeOneCharacter.ex();
        }

        // check that escape has exactly one character
        if (escape.length() != 1) {
            throw ApplibResource.instance().EscapeCharMustBeOneCharacter.ex();
        }
        return true;
    }
}

...
public abstract class FlattenRecursiveHierarchyUdx
{
     ...
     * column contains child nodes) and output to another table. Each row of the
     * output table represent a flattened path from a root to a leaf. The first
     * column contains integer values representing the actual number of vertices
     * on the path. The second column contains boolean values true if there
     * exists multiple paths that end at the leaf node of the path. Third to
     * last columns contain vertices along the path starting from the root
     * ending at the leaf. If the number of vertices is less than maxDepth, the
     * path is right-padded with value of the leaf. If the number of vertices is
     * greater than maxDepth, the path is truncated (actual number of vertices
     * on the path before truncate is shown in first column).
     *
    ...
    /**
     * Same as execute, but returns rows for non-leaf vertices as well. Produces
     * one extra column (after the first two fixed columns) containing a boolean
     * indicating whether a given output row corresponds to a non-leaf vertex.
     *
     * @param inputSet input table
     * @param maxDepth nominal maximal depth of paths
     * @param resultInserter output table
        ...
        throws ApplibException
    {
        ...
        HashMap<String, ArrayList<ArrayList<String>>> pathsFound =
            new HashMap();

        try {
            buildGraphFromInput(inputSet, inGraph);
        } catch (SQLException e) {
            throw ApplibResource.instance().CannotReadInput.ex(e);
        }
        ...
        if (cyc.detectCycles()) {
            ...
            throw ApplibResource.instance().GraphHasCycle.ex(
                String.valueOf(orderedVerticesInLoop.size()),
                sb.toString());
        }
        ...
        throws ApplibException, SQLException
    {
        ...
        while (inputSet.next()) {
            ...
            if (child == null) {
                throw ApplibResource.instance().NullChild.ex(parent);
            } else {
        ...
        throws ApplibException
    {
            ...
            if (numParents == 1) {
                ...
                for (i = 0; i < pathList.size(); i++) {
                    ...
                    try {
                        ...
                        throw ApplibResource.instance().CannotWriteOutput.ex(
                            e);
                    }
                }
                return;
        ...
        try {
            ...
            throw ApplibResource.instance().CannotWriteOutput.ex(e);
        }
    }

    /**
     * Takes a list representing vertices in a path and sends them to one row of
     * the output table
     *

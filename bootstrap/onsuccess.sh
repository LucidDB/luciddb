...
INSTALLER=${ANS:2}
ANS=`find -name "SQLstream*ClientTools.zip" | sort -r | head -n 1`
CLIENTTOOLS=${ANS:2}
...
# create link to installer, client tools, and text file with the link target 
# so scp clients can determine what they've downloaded
rm -f $LINKINFO
ln -sf $INSTALLER $LINKNAME
ln -sf $CLIENTTOOLS $LINKCLIENT
echo $INSTALLER >$LINKINFO

# create MD5 signature file
rm -f $MD5INFO
DIR=$PWD
(cd `dirname $INSTALLER`; md5sum `basename $INSTALLER` `basename $CLIENTTOOLS` >$DIR/$MD5INFO)

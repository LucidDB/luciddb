...
/**
 * FennelNestedLoopJoinRule is a rule for converting a {@link JoinRel} with a
 * join condition into a {@link FennelNestedLoopJoinRel}. The nested loop join
 ...
 *
 * <p>Any sargable join predicates that aren't part of the index lookup are
 * processed by a {@link FennelReshapeRel}. And then finally, any remaining join
    ...
    extends RelOptRule
{
     ...
     *
     * <p>In the new tree, the first input is the left join input. The left join
     * input is assumed to have been casted, as needed, to match the right join
     * input.
     *
     * <p>The right input does all the necessary lookups and filtering on the
     * right input by reading dynamic parameters corresponding to the left
     * input.
     *
     ...
     * @param indexCols list of column offsets from the RHS input that are to be
     * indexed
     * @param indexOperands list of column offsets from the LHS input that will
     * be used as index lookup keys
     * @param indexOp the operator to be used in the index lookup
     * @param filterCol the column offset from the RHS input that can be applied
     * in FennelReshapeRel
     ...
     * FennelReshapeRel
     * @param residualCondition remaining filter that must be applied in a Calc
     * node
     *
     ...
     * range predicate for use with a temporary index and then if possible, an
     * additional range predicate for a FennelReshapeRel
     *
     * @param joinInputs join inputs
     * @param joinCondition the join condition
     * @param indexCols returns list of column offsets from the right join input
     * that are to be indexed
     * @param indexOperands returns list of column offsets from the left join
     * input that will be used as index lookup keys
     * @param indexOp returns the operator to be used in the index lookup
     * @param filterCol returns the column offset corresponding to RHS input
     * that can be filtered in FennelReshapeRel
     * @param filterOperand returns the column offset corresponding to LHS input
     * that filterCol is compared against
        ...
        List<Integer> outputProj)
    {
        ...
        if (extraPreds != null) {
            ...
            if (adjustment > 0) {
                int nTotalFields =
                    nLeftFieldsOrig
                    + joinInputs[1].getRowType().getFieldCount();
     ...
     * @param joinKeyParamMap mapping from join keys to dynamic parameters; used
     * to keep track of which keys already have corresponding dynamic parameters
     */
     ...
     *
     * @param residualCondition remaining join condition to be processed by Calc
     * node
     * @param nLeftFields number of fields in the left join input
     * @param leftInputRefs list of column offsets corresponding to left input
     * references
     */
        ...
        List<Integer> leftInputRefs)
    {
        ...
        for (
            int field = inputRefs.nextSetBit(0);
            field >= 0;
     ...
     * the RHS input. If a temporary index is used in the join lookup, create an
     * index lookup RelNode. The output from either that (or the RHS input, if
     * no temp index lookup is required) is then optionally fed into a
     * FennelReshapeRel and/or a Calc node for additional join filtering.
     *
     * @param leftRel left input into the new join
     * @param rightRel right input into the new join
     * @param indexCols list of column offsets from the RHS input that are to be
     ...
     * node
     *
     * @return RelNode tree corresponding to the nested loop join's second input
     * or null if traits couldn't be converted
     */
        ...
        FennelRelParamId rootPageIdParamId)
    {
        ...
        RelDataTypeFactory typeFactory = rightRel.getCluster().getTypeFactory();
     ...
     * @param joinKeyParamMap mapping from LHS inputs to dynamic parameters
     * @param filterCols the column offset from the RHS input that can be
     * applied in FennelReshapeRel
        ...
        Map<Integer, Integer> residualRefMap)
    {
        ...
        for (Integer residualRef : residualRefs) {
            if (!filterOperands.contains(residualRef)) {
                ...
                fieldNames[currOutputIdx++] = leftFields[residualRef].getName();
            }
        }

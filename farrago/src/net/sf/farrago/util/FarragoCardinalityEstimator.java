...
/**
 * FarragoCardinalityEstimator estimates the number of distinct values
 * in a population based on a sample of that population.  The algorithms in
 * this class are adapted from "Estimating the Number of Classes in a Finite 
 * Population" by Peter J. Haas and Lynne Stokes (IBM Research Journal;
 * RJ 10025; May 1996).
 *
...
public class FarragoCardinalityEstimator
{
     ...
     * appear fewer than {@link #Duj2a_DIVIDER} times.  Falls in the range 
     * [0, dn].
     */
     ...
     * maximum index in this array is {@link #Duj2a_DIVIDER} - 1.  May contain
     * no entries.
     */
     ...
     * {@link #Duj2a_DIVIDER} times.  May be empty or have as many entries
     * as calls to {@link #addSampleClass(long, boolean)} (if all classes
     * appear more than Duj2a_DIVIDER times.
     */
    private List<Long> B;
     ...
     * added in any particular order.  The goal of this method is to compute 
     * the data structures necessary to estimate cardinality of the sample 
     * without requiring multiple passes through the sample itself.
     * 
     * @param classSize the number of times the class appears
     * @param isNullClass set true if this class has no value (may only be
     *                    done once per sample)
     * 
    ...
    /**
     * Estimates the cardinality of the population with the assumption that
     * all classes except the null class are distinct.
     */
    ...
    /**
     * Estimates the cardinality of the population using the formulas
     * described {@link FarragoCardinalityEstimator above}.
     */
    ...
    private static double estimateVarianceFromD(
        long n, long N, SparseLongArray f, long D)
    {
        // Estimate gamma^2(D): the variance of the class sizes Nj
        // Eq 16: y^2(D) = max(0, D/n^2 * sum(i = 1 to n: i*(i - 1)*fi) + (D/N) - 1
        long sum = 0;
        for(long i = 1; i <= n; i++) {
            long fi = f.get(i);
            sum += i * (i - 1) * fi;
    ...
    private static long computeDuj2(
        long n, long dn, long f1, double q, double ysqD)
    {
    ...
    private double compute_nj(
        final double q, final double _1q, double Nj)
    {

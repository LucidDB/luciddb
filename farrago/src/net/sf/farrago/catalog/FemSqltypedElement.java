 ...
 * CwmSqlparameter.  Validation code would like to be able to handle any kind
 * of SQL-typed object uniformly, so a common interface is required.
 ...
 * generation of operations, so we can't declare a UML interface with
 * abstract methods.
 *
 *<p>
 *
 * So, we define this interface outside the model and use proxies to implement
 * it; see {@link FarragoCatalogUtil#toFemSqltypedElement}.  In the model, we
 * define a placeholder interface {@link FemAbstractTypedElement}.
 * 
 * @author John V. Sichi
 * @version $Id$
 */

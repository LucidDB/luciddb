 ...
 * Farrago column set. An instance of this class is returned to summarize 
 * the result of applying predicate(s) to a column.
 * 
 * <p>TODO: Review statistics analysis for handling of null semantics. 
 * Null values are less than all other values (e.g. bars might contain the 
 * starting values null,0,1,...). Because stats analysis is based on ranges, 
 * only ranges which include consecutive bars are supported. Examples:
 * 
 *  <ul>
 *    <li>if NULL_MATCHES_ANYTHING, and querying for "col = 5", then match 
 *        either null or 5. Two intervals are required [null,null] and [5,5]
 *    <li>if NULL_MATCHES_NULL, and querying "col = null" then null may be 
 *        matched with the point interval [null,null]
 *    <li>otherwise, null cannot be matched, so a query like "col &lt; 1" 
 *        becomes (null,1)
 *  </ul>
 * 
 * @author John Pham
 * @version $Id$
 */
public class FarragoColumnHistogram implements RelStatColumnStatistics
{
    ...
    /**
     * Initializes a column statistics reader. The statistics are not 
     * actually analyzed until the user calls {@link #evaluate()}.
     * 
    ...
    /**
     * Analyzes column histogram to determine the selectivity and 
     * cardinality of the specified search condition. 
     */
    protected void evaluate()
    {
        ...
        assert (bars.size() == barCount)
            : "invalid histogram bar count";
    ...
    /**
     * Computes the histogram bar coverage of an ordered sequence of 
     * intervals.
     * 
    ...
    /**
     * Reads collective coverages finally make estimates on 
     * requested attributes. This implementation looks at each 
     * bar separately, estimating how much of each bar is covered. 
     * It then accounts for that bar's contribution to the entire 
     * results.
     * 
    ...
    private void readCoverages(List<HistogramBarCoverage> coverages)
    {
        ...
        Double correction = NumberUtil.divide(
            Double.valueOf(histValues), Double.valueOf(sampleValues));
        
        Double totalFraction = 0.0;
        Double totalValues = 0.0;
        for (int i = 0; i < coverages.size(); i++) {
            // estimate the actual cardinality of the bar
            Double barValues = NumberUtil.multiply(
                Double.valueOf(bars.get(i).getValueCount()), correction);
        ...
        totalFraction = 
            NumberUtil.divide(totalFraction, (double)bars.size());
    ...
    private class HistogramRange
    {
        ...
        /**
         * Searches through the histogram to find the first and last bars 
         * the interval may cover in the histogram. This range may be empty.
         */
        protected void evaluate()
        {
            int start = findStartBar(minBar, interval.getLowerBound());
            int end = findEndBar(start, interval.getUpperBound());
        ...
        /**
         * Returns the zero based index of the first bar in the range, 
        ...
        /**
         * Returns the zero based index of the last bar in the range,
         * or -1 if the range is empty.
         */
         ...
         * starting search point, or points greater than the search point. 
         * We can rule out histogram bars which end before the search point.
         * However, we can never rule out the last histogram bar, because 
        ...
        private int findStartBar(int min, SargEndpoint point) 
        {
            ...
            for (start = min; start + 1 < bars.size(); start++) {
                ...
                int comparison = compare(nextBar.getStartingValue(), coordinate);
         ...
         * <p>
         * 
         * If all bars may contain the search point, returns the index of the 
         * last bar + 1.
         * 
         * @param min index of first histogram bar to search
        ...
        private int findEndBar(int min, SargEndpoint point) 
        {
            ...
            for (end = min; end < bars.size(); end++) {
                ...
                int comparison = compare(bar.getStartingValue(), coordinate);
                if ((comparison > 0) || (open && comparison == 0)) {
                    break;
                }
            }
    ...
    private static class HistogramBarCoverage
    {
            ...
            String minVal)
        {
            ...
            if (!lower.isFinite()) 
            {
                assert (range.firstBar == 0) 
                    : "-infinity does not span first histogram bar";
                ...
                if (comparison == 1 || comparison == 0 && lower.isClosed()) {
                    coverages.get(0).entireBar = true;
                }
            }
            ...
            {
                assert (range.lastBar == coverages.size() - 1)
                    : "+infinity does not span last histogram bar";
                coverages.get(range.lastBar).entireBar = true;
            }
        }

        /**
         * Estimates what percentage of a bar is covered, based on the 
         * bar's total cardinality.
         * 
         * @param cardinality the expected cardinality of the bar
         * 
         * @return an estimate from 0.0 to 1.0 representing a fraction 
         * of a bar, or null if no reliable estimate can be made
         */
        ...
        /**
         * Estimates the cardinality of the column after applying 
         * a predicate. 
         * 
         * @param cardinality the expected cardinality of the bar
         * 
         * @return an estimate of the cardinality of the column 
         * after applying the predicate, or null if no reliable 
        ...
        protected Double estimateCardinality(Double cardinality)
        {
            ...
            assert (estimatedBarCardinality >= 0) 
                : "histogram bar contained negative number of distinct values";

 ...
 * performing a regular row scan). It may have the benefit of a sorted 
 * result set.
 *
 * @author John Pham
 * @version $Id$
 */
public class LcsIndexOnlyScanRel extends FennelOptionalRel
{
    ...
    /**
     * Constructs an index only scan based upon another index scan. The 
     * index scan already has an input.
     *
     * @param cluster the environment for the scan
     * @param child the index scan input, which produces search directives
     * @param searchRel the index search to be converted into an index only 
     *   scan
    ...
    public LcsIndexOnlyScanRel(
        ...
        Integer[] proj)
    {
        this(cluster, child, searchRel.lcsTable, index, 
            searchRel.fullScan, proj, 
            searchRel.isUniqueKey, searchRel.isOuter, searchRel.inputKeyProj,
            searchRel.inputJoinProj, searchRel.inputDirectiveProj);
    }
     ...
     * @param index the index to be scanned
     * @param projectedColumns for full scans, the output projection. 
     *   Should be null for searches
     * @param isUniqueKey
     * @param isOuter
     * @param inputKeyProj for searches, the key columns
     * @param inputJoinProj for index joins applied to searches, the input 
     *   keys to be output
     * @param inputDirectiveProj for searches, the search directive columns
     * @param outputProj for searches, the output projection. Should be 
     *   null for full scans
     */
    ...
    {
        LcsIndexOnlyScanRel clone = new LcsIndexOnlyScanRel(
            getCluster(), getChild(), table, index, fullScan,
            projectedColumns, isUniqueKey, isOuter, inputKeyProj, 
            inputJoinProj, inputDirectiveProj);
    ...
    {
        RelDataType rowType = 
            table.getIndexGuide().createUnclusteredRowType(index, projectedColumns);
        return rowType;
    }
    ...
    public void explain(RelOptPlanWriter pw)
    {
        ...
        Object inputKeyProjObj =
            FennelRelUtil.explainProjection(inputKeyProj);
    ...
    public FemExecutionStreamDef toStreamDef(FennelRelImplementor implementor)
    {
        ...
        LcsIndexSearchRel searchRel = new LcsIndexSearchRel(
            getCluster(),
            getChild(),
            table,
            index,
        ...
        LcsIndexMinusRel minus = indexGuide.createMinusOfDeletionIndex(
            this, searchRel.lcsTable, searchRel);

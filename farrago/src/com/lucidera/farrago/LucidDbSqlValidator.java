...
public class LucidDbSqlValidator extends FarragoSqlValidator
{
    ...
    private void validateMergeUniqueKeys(SqlMerge call)
    {
        ...
        for (SqlNode updateCol : updateList) {
            ...
            if (uniqueCols.get(columnNum)) {
                ...
                {
                    throw 
                        FarragoResource.instance().ValidatorUniqueKeyUpdate.ex(
                            columnName);
                }
            }
        }
     ...
     * or if it's being updated, the update is a no-op because the column
     * is either being set to itself or the column it is being equi-joined with
     * in the ON clause of the MERGE statement
     * 
     ...
     * 
     * @return true if the unique column is not updated or is updated to a
     * value that does not change the column value
     */
        ...
        SqlValidatorScope scope)
    {
        ...
        if (sourceExpr.isA(SqlKind.Cast)) {
            ...
            RelDataType castType =
                getValidatedNodeType(sourceExpr);
            ...
            sourceType = typeFactory.createTypeWithNullability(
                sourceType, true);
     ...
     */
    private class EquiJoinFinder extends SqlBasicVisitor<Void>
    {

 ...
 * it assumes that {@link LoptModifyRemovableSelfJoinRule} has already
 * been applied.
 *
 * @author Zelaine Fong
 * @version $Id$
    ...
    extends RelOptRule
{
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        RelNode newFilter =
            combineFilters(leftFilter, rightFilter, newRowScan);
    ...
    /**
     * Combines two row scans into one by taking the intersection of the 
     * inputs into each row scan.  It's assumed that each row scan has no more
     * than one input.
     * 
     ...
     * 
     * @return a FilterRel containing the combined filters on top of the
     * new row scan; if both filters are always true, then the combined row
     * scan is returned instead
     */
    ...
    /**
     * Combines projections on top of filters that are top of row scans into
     * a single projection.  Since the row scans are identical and have not
     * yet been projected, the original expressions in both projections
     * will be the same in the combined projection.
     * 
    ...
    /**
     * Replaces the join with whatever additional filters need to be applied
     * as a result of removing the join.
     * 
     * @param joinRel the original join
     * @param joinInput the new input into the join
     * 
     * @return a FilterRel containing whatever additional filters need to
     * be applied; if no additional filtering is required, then the new join
     * input is returned
     */
        ...
        ProjectRel joinInput)
    {
        ...
        for (Integer key : leftKeys) {
            ...
            if (fields[key].getType().isNullable()) {
                RexNode[] expr =
                    new RexNode[]
                        { RelOptUtil.createInputRef(joinInput, key) };
                extraFilterList.add(
                    rexBuilder.makeCall(

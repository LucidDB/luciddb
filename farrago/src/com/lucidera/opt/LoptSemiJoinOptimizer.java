...
public class LoptSemiJoinOptimizer
{
            ...
            while (dimIter.hasNext()) {
                ...
                if (joinFilters != null) {
                    SemiJoinRel semiJoin = findSemiJoinIndex(
                        multiJoin, indexGuide, joinFilters, factIdx, dimIdx);
    ...
    private int isSuitableFilter(
        LoptMultiJoin multiJoin, RexNode joinFilter, int factIdx)
    {
        ...
        int factIdx, int dimIdx)
    {
        ...
        semiJoinCondition = adjustSemiJoinCondition(
            multiJoin, leftAdjustment, semiJoinCondition, factIdx, dimIdx);
        ...
        RelOptUtil.splitJoinCondition(
            factRel, dimRel, semiJoinCondition, leftKeys, rightKeys);
        ...
        if (leftKeys.size() == keyOrder.size()) {
            ...
            semiJoinCondition = removeExtraFilters(
                actualLeftKeys, multiJoin.getNumFieldsInJoinFactor(factIdx),
                semiJoinCondition);
        }
        SemiJoinRel semiJoin = new SemiJoinRel(
            factRel.getCluster(), factRel, dimRel, semiJoinCondition,
            actualLeftKeys, actualRightKeys);
        return semiJoin;
    }
    ...
    private RexNode adjustSemiJoinCondition(
        LoptMultiJoin multiJoin, int leftAdjustment, RexNode semiJoinCondition,
        ...
        if (leftAdjustment != 0 || rightAdjustment != 0) {
            ...
            if (rightAdjustment != 0) {
                for (int i = rightStart;
                    i < rightStart + numFieldsRightIdx; i++)
                {

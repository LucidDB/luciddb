...
public class HepProgramBuilder
{
     ...
     * RelOptPlanner#addRule}.  If the planner does not have any rules
     * of the given class, this instruction is a nop.
     *
     *<p>
     *
    ...
    /**
     * Adds an instruction to attempt to match a specific rule
     * identified by its unique description.
     *
     *<p>
     *
     * Note that when this method is used, it is necessary to also add the rule
     * object of interest to the planner via {@link RelOptPlanner#addRule}.
     ...
     * the actual instances.  If the planner does not have a rule
     * matching the description, this instruction is a nop.
     *
     * @param rule rule to fire
     */
    ...
    /**
     * Adds an instruction to begin a group of rules.  All subsequent rules
     * added (until the next endRuleGroup) will be collected into the group
     * rather than firing individually.  After addGroupBegin
     * has been called, only addRuleXXX methods may be called until the next
     * addGroupEnd.
     */
    ...
    public void addMatchOrder(HepMatchOrder order)
    {
        ...
        HepInstruction.MatchOrder instruction =
            new HepInstruction.MatchOrder();
        instruction.order = order;
        instructions.add(instruction);
    }

    /**
     * Adds an instruction to limit the number of pattern matches for
     * subsequent instructions.  The limit will take effect for the rest of the
     * program (not counting subprograms) or until another limit instruction is
     * encountered.
     *
     * @param limit limit to set; use {@link HepProgram#MATCH_UNTIL_FIXPOINT}
     * to remove limit
     */
    public void addMatchLimit(int limit)
    {
        ...
        HepInstruction.MatchLimit instruction =
            new HepInstruction.MatchLimit();
        instruction.limit = limit;
        instructions.add(instruction);
    }

    /**
     * Adds an instruction to execute a subprogram.  Note that this is
     * different from adding the instructions from the subprogram individually.
     * When added as a subprogram, the sequence will execute repeatedly until a
     * fixpoint is reached, whereas when the instructions are added
     * individually, the sequence will only execute once (with a separate
     * fixpoint for each instruction).
     *
     *<p>
     *
     * The subprogram has its own state for match order and limit (initialized
     * to the defaults every time the subprogram is executed) and any changes
     * it makes to those settings do not affect the parent program.
     *
    ...
    public void addSubprogram(HepProgram program)
    {
        ...
        HepInstruction.Subprogram instruction =
            new HepInstruction.Subprogram();
        instruction.subprogram = program;
        instructions.add(instruction);
    }

    /**
     * Returns the constructed program, clearing the state of
     * this program builder as a side-effect.
     *

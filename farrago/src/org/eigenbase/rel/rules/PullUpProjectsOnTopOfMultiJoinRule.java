 ...
 * {@link ProjectRel}s that are on top of a {@link MultiJoinRel} and beneath a
 * {@link JoinRel} so the {@link ProjectRel} appears above the {@link JoinRel}.
 * In the process of doing so, also save away information about the respective
 * fields that are referenced in the expressions in the {@link ProjectRel}
 * we're pulling up, as well as the join condition, in the resultant
 * {@link MultiJoinRel}s 
 *
 ...
 *
 * <p>The {@link MultiJoinRel} associated with (X, Y) associates x1 with
 * X and y1 with Y.  Although x3 and y3 need to be read due to the
 * filters, they are not required after the row scan has completed and
 * therefore are not saved.  The join fields, x2 and y2, are also tracked
 * separately.

 * <p>Note that by only pulling up projects that are on top of
 * {@link MultiJoinRel}s, we preserve projections on top of row scans.
 * 
 * <p>See the superclass for details on restrictions regarding which
 * {@link ProjectRel}s cannot be pulled.
 *
 * @author Zelaine Fong
 * @version $Id$
 */

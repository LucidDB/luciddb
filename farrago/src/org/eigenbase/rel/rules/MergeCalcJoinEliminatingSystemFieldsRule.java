 ...
 * without system fields, if a {@link CalcRel} above the join indicates that
 ...
 *
 * @see MergeProjectJoinEliminatingSystemFieldsRule
 *
 * @author Julian Hyde
 * @version $Id$
 */
public final class MergeCalcJoinEliminatingSystemFieldsRule
    extends RelOptRule
{
    public static final MergeCalcJoinEliminatingSystemFieldsRule instance =
        new MergeCalcJoinEliminatingSystemFieldsRule();

    //~ Constructors -----------------------------------------------------------

    /**
     * Creates a MergeCalcJoinEliminatingSystemFieldsRule.
     */
    private MergeCalcJoinEliminatingSystemFieldsRule()
    {
        super(
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        // Are we projecting any of the system fields, or using them in other
        // expressions?
        final int fieldCount = joinRel.getRowType().getFieldCount();
        final RexProgram program = calcRel.getProgram();
        final int[] refCounts = program.getReferenceCounts();
        ...
        final RexProgram newProgram =
            program.permuteInputs(
                calcRel.getCluster().getTypeFactory(),
                mapping);
        final CalcRel newCalcRel =
            new CalcRel(
                calcRel.getCluster(),
                calcRel.getTraits(),
                newJoinRel,
                ...
                newProgram,
                RexUtil.apply(mapping, calcRel.getCollationList()));
...
// End MergeCalcJoinEliminatingSystemFieldsRule.java

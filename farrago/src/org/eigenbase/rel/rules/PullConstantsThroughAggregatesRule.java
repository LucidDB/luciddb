 ...
 * of the group list, removing it will leave a hole. In this case, the rule
 * adds a project before the aggregate to reorder the columns, and permutes
...
public class PullConstantsThroughAggregatesRule extends RelOptRule
{
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        if (constantList.get(0) == newGroupCount) {
            ...
            final RelNode project =
                createProjection(mapping, child);
        ...
        for (int i = 0, source = 0; i < aggregateRowType.getFieldCount();
             ++i) {
            RexNode expr;
            if (i >= groupCount) {
                // Aggregate expressions' names and positions are unchanged.
                expr = RelOptUtil.createInputRef(
                    newAggregate, i - constantList.size());
    ...
    private static RelNode createProjection(
        final Mapping mapping, RelNode child)
    {

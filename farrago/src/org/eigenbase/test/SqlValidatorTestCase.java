...
public abstract class SqlValidatorTestCase extends TestCase {
    ...
    public void checkFails(String sql, String expected)
    {
    ...
    private RelDataType getResultType(String sql) {
        ...
        RelDataType actualType = validator.getValidatedNodeType(((SqlNodeList)((SqlCall)n).getOperands()[1]).get(0));
        return actualType;
    }
...
//                                "some error msg with line + col");
    ...
    public void testEqualNotEqualFails() {
        ...
        checkExpFails("b'1'=0.01"    ,"(?s).*Can not apply '=' to arguments of type '<BIT.1.> = <DECIMAL.3, 2.>'.*");
        checkExpFails("b'1'=1"    ,"(?s).*Can not apply '=' to arguments of type '<BIT.1.> = <INTEGER>'.*");
        checkExpFails("b'1'<>0.01"    ,"(?s).*Can not apply '<>' to arguments of type '<BIT.1.> <> <DECIMAL.3, 2.>'.*");
        checkExpFails("b'1'<>1"    ,"(?s).*Can not apply '<>' to arguments of type '<BIT.1.> <> <INTEGER>'.*");
    }

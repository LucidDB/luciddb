    ...
    extends SqlToRelTestBase
{
    ...
    public void testJoinValues()
    {
        check(
            "select *\n"
            + "from (values (1)) as va(i),\n"
            + "(values (4, 4)) as vb(x, j) where j = x",
            "${plan}");
    }
    ...
    /**
     * Tests a query similar to that generated by
     * {@link org.eigenbase.sql.test.SqlOperatorTests#testSumFunc()}.
     */
    public void testGroupUnionValues()
    {
        check(
            "SELECT sum(x) FROM ("
            + "SELECT 0 AS x FROM (VALUES (1)) UNION ALL "
            + "SELECT CAST(null AS INTEGER) AS x FROM (VALUES (1)) UNION ALL "
            + "SELECT 2 AS x FROM (VALUES (1)) UNION ALL "
            + "SELECT 2 AS x FROM (VALUES (1)))",
            "${plan}");
    }
    ...
    {
        // having clause contains an uncorrelated subquery
        check(
            "select sum(sal + sal) from emp\n"
            + "group by deptno\n"
            + "having sum(sal) > (select sum(sal - 1) from emp)",
            "${plan}");
    }
    ...
    public void testGroupBug281b()
    {
        ...
        tester.assertConvertsTo(sql, "${plan}");
        tester.assertConvertsTo(sql, "${planTrimmed}", true);
    }
    ...
    {
        final String sql = "select empno+1 from sales.emp order by empno desc";
        tester.assertConvertsTo(sql, "${plan}");
        tester.assertConvertsTo(sql, "${planTrimmed}");
    }
    ...
    public void testSubqueryEquals()
    {
        check(
            ...
            + "  select min(deptno) from emp where slacker)\n"
            + "order by name",
            "${plan}");
    }
    ...
    /**
     * Same query as {@link #testInUncorrelatedSubquery()}, but trim unused
     * columns.
     */
    public void testInUncorrelatedSubqueryTrimmed()
    {
    ...
    /**
     * Tests that a multi-column, multi-record VALUES clause can be implemented
     * by a single FennelValuesRel.
     */
    ...
    /**
     * As {@link #testValues}, but VALUES also contains a NULL value.
     */
    public void testValuesWithNull()
    {
        check(
            "VALUES (1, 'a'), (2, 'b'), (CAST(NULL AS INTEGER), 'cd')",
            "${plan}");
    }

    /**
     * As {@link #testValuesWithNull}, but in a subquery, which allows us to
     * alias the columns.
     */
    public void testValuesWithNullInSubquery()
    {
        check(
            "SELECT y, x FROM (\n"
            + " VALUES (1, 'a'), (2, 'b'), (CAST(NULL AS INTEGER), 'cd')\n"
    ...
    public void testValuesTrimmed()
    {
        final String sql =
            "SELECT d, b FROM (\n"
            + "  VALUES\n"
            + "    (1, 2, 3, 4),\n"
            + "    (5, 6, 7, 8)) AS t (a, b, c, d)";
        tester.assertConvertsTo(sql, "${plan}");
        tester.assertConvertsTo(sql, "${planTrimmed}", true);
    }
    ...
    public void testProjectOnProjectTrimmed()
    {
        final String sql =
    ...
    public void testProjectOnFilterTrimmed()
    {
        final String sql =
            "select empno + deptno from (\n"
            + "  select * from emp)\n"
    ...
    public void testUnionTrimmed()
    {
        final String sql =
            "select a + 3 from (\n"
            + "  select empno as a, sal as b, ename as c from emp\n"
            + "  union all\n"
            + "  select deptno, deptno, name from dept)\n"
            + "where a > b";
        tester.assertConvertsTo(sql, "${plan}", false);
        tester.assertConvertsTo(sql, "${planTrimmed}", true);
    }

    /**
     * Tests field trimming in the case where none of the output fields of
     * a UNION are used. Usually a relational operator will add a dummy field,
     * but UNION is tricky because it inherits fields from its inputs, and those
     * types must be consistent.
     */
    public void testUnionEmptyTrimmed()
    {
        final String sql =
            "select count(*) from (\n"
            + "  select empno as a, sal as b, ename as c from emp\n"
    ...
    public void testJoinSubqueryTrimmed()
    {
        final String sql =
            "select d.deptno + e.sal\n"
            + "from (select * from emp) as e\n"
            + "join (select * from dept) as d on e.deptno = d.deptno\n"
            + "where e.deptno > 10";
        tester.assertConvertsTo(sql, "${plan}");
        tester.assertConvertsTo(sql, "${planTrimmed}", true);
    }

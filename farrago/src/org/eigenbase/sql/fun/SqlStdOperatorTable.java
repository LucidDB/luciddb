...
public class SqlStdOperatorTable extends SqlOperatorTable {
    ...
    public final SqlBinaryOperator concatOperator =
            new SqlBinaryOperator("||", SqlKind.Other, 30, true,
                    useNullableVaryingDyadicStringSumPrecision, null,
                    typeNullableStringStringOfSameType) {
...
//                    tester.checkNull("x'ff' || cast(null as varbinary)");
    ...
    public final SqlInternalOperator litChainOperator =
                               ...
                               useFirstArgType, useFirstKnownParam, null)
        {
            ...
            private boolean argTypesValid(SqlCall call, SqlValidator validator,
                                          SqlValidator.Scope scope)
            {
                ...
                for (int i = 1; i < call.operands.length; i++) {
                    ...
                    RelDataType otherType = validator.deriveType(scope, operand);
            ...
            protected RelDataType inferType(SqlValidator validator,
                ...
                RelDataType rt = validator.getValidatedNodeType(call.operands[0]);
                SqlTypeName tname = rt.getSqlTypeName();
                assert tname.allowsPrecNoScale() : "LitChain has impossible operand type "+tname;
            ...
            void validateCall(SqlCall call, SqlValidator validator) {
                ...
                for (int i = 1; i < call.operands.length; i++, prevPos = pos) {
                    ...
                    if (pos.getBeginLine() <= prevPos.getBeginLine())
                        ...
                        throw EigenbaseResource.instance().newValidationError(
            ...
            public void unparse(SqlWriter writer, SqlNode[] rands, int leftPrec, int rightPrec) {
                ...
                for (int i = 0; i < rands.length; i++) {
                    ...
                    if (rand instanceof SqlLiteral.CharString) {
                        NlsString nls = ((SqlLiteral.CharString) rand).getNlsString();
                        if (i == 0) {
            ...
            public void test(SqlTester tester) {
                    ...
                    tester.checkString("'corned'\n' beef'\n' on'\n' rye'", "corned beef on rye");
                    tester.checkString("_latin1'Spaghetti'\n' all''Amatriciana'", "Spaghetti all'Amatriciana");
    ...
    public final SqlBetweenOperator betweenOperator =
            new SqlBetweenOperator(
                    SqlBetweenOperator.Flag.createAsymmetric(ParserPosition.ZERO), false);

    public final SqlBetweenOperator symmetricBetweenOperator =
            new SqlBetweenOperator(
                    SqlBetweenOperator.Flag.createSymmetric(ParserPosition.ZERO), false);

    public final SqlBetweenOperator notBetweenOperator =
            new SqlBetweenOperator(
                    SqlBetweenOperator.Flag.createAsymmetric(ParserPosition.ZERO), true);

    public final SqlBetweenOperator symmetricNotBetweenOperator =
            new SqlBetweenOperator(
                    SqlBetweenOperator.Flag.createSymmetric(ParserPosition.ZERO), true);
    ...
    public final SqlFunction substringFunc =
            new SqlFunction("SUBSTRING", SqlKind.Function,
                    SqlOperatorTable.useNullableVaryingFirstArgType, null,
                    null, SqlFunction.SqlFuncTypeName.String) {

                protected String getSignatureTemplate(final int operandsCount) {
                    switch (operandsCount) {
                    case 2: return "{0}({1} FROM {2})";
                    case 3: return "{0}({1} FROM {2} FOR {3})";
                    }
                    assert(false);
                    return null;
                }
...
//                    tester.checkString("substring('foobar' from '%#\"o_b#\"%' for '#')", "oob");
    ...
    public final SqlFunction overlayFunc =
            new SqlFunction("OVERLAY", SqlKind.Function,
                    SqlOperatorTable.useNullableDyadicStringSumPrecision, null,
                    null, SqlFunction.SqlFuncTypeName.String) {
                ...
                public String getAllowedSignatures(String name) {
                    ...
                    for (int i = 0; i < SqlOperatorTable.stringTypes.length; i++) {
                        ...
                        ArrayList list = new ArrayList();
                        list.add(SqlOperatorTable.stringTypes[i]);
                        list.add(SqlOperatorTable.stringTypes[i]); //adding same twice
                        list.add(SqlTypeName.Integer);
                        ret.append(this.getAnonymousSignature(list));
                        ret.append(NL);
                        list.add(SqlTypeName.Integer);
                ...
                protected String getSignatureTemplate(final int operandsCount) {
                    switch (operandsCount) {
                    case 3: return "{0}({1} PLACING {2} FROM {3})";
                    case 4: return "{0}({1} PLACING {2} FROM {3} FOR {4})";
                    }
                    assert(false);
                    return null;
                }
...
//                    tester.checkNull("initcap(cast(null as varchar))");
    ...
    public final SqlFunction modFunc =
            new SqlFunction("MOD", SqlKind.Function,
                    ...
                    SqlOperatorTable.typeNullableIntegerInteger,
                    SqlFunction.SqlFuncTypeName.Numeric) {
                public void test(SqlTester tester) {
                ...
                public SqlCall createCall(SqlNode[] operands, ParserPosition pos) {
                    if (2 != operands.length) {
                        ...
                        throw EigenbaseResource.instance().
                                newInvalidNbrOfArgument(name,
                                    pos.toString(),new Integer(2));
                    }
                    ...
                    return caseOperator.createCall(
                            operands[0], whenList, thenList, operands[0],pos);
                }
...
//                    tester.checkNull("nullif(cast(null as varchar),'a')");
    ...
    public final SqlFunction coalesceFunc =
            new SqlFunction("COALESCE", SqlKind.Function, null, null, null,
                ...
                private SqlCall createCall(SqlNode[] operands, int start, ParserPosition pos) {
                    SqlNodeList whenList = new SqlNodeList(pos);
                    SqlNodeList thenList = new SqlNodeList(pos);
                    ...
                    if (2 == (operands.length - start)) {
                        return caseOperator.createCall(
                                null, whenList, thenList, operands[start + 1],pos);
                    }
                    return caseOperator.createCall(
                            null, whenList, thenList,
                            this.createCall(operands, start + 1, pos),pos);
                }
...
//                    tester.checkScalarExact("coalesce(null,null,3)", "3");
    ...
    public final SqlFunction localTimeFunc =
            new SqlFunction("LOCALTIME", SqlKind.Function, null, null,
                ...
                protected RelDataType inferType(SqlValidator validator,
                    ...
                    if (precision < 0) {
                        throw EigenbaseResource.instance().newArgumentMustBePositiveInteger(call.operator.name);
                    }
                    RelDataType type = validator.typeFactory.createSqlType(
                ...
                public RelDataType getType(RelDataTypeFactory typeFactory,
                    ...
                    RelDataType type = useTime.getType(typeFactory,argTypes);
    ...
    public final SqlFunction currentDateFunc =
            new SqlFunction("CURRENT_DATE", SqlKind.Function, null, null,
                ...
                public RelDataType getType(SqlValidator validator, SqlValidator.Scope scope,
                        SqlCall call) {
                    int precision = 0; // todo: access the value of the literal parmaeter
                    return validator.typeFactory.createSqlType(SqlTypeName.Time, precision);
                }

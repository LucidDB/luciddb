...
public class SqlOperatorTable
{
    ...
    public static final SqlOperator.TypeInferenceTransform transformVarying =
            new SqlOperator.TypeInferenceTransform() {
                public RelDataType getType(RelDataTypeFactory fac,
                        RelDataType[] argTypes, RelDataType type) {
                    ...
                    RelDataType ret = fac.createSqlType(retTypeName,
                            type.getPrecision());
                    if (type.isCharType()) {
    ...
    public static final SqlOperator.TypeInference useFirstArgType =
            new SqlOperator.TypeInference() {
                ...
                public RelDataType getType(SqlValidator validator, SqlValidator.Scope scope, SqlCall call) {
                ...
                public RelDataType getType(RelDataTypeFactory typeFactory,
                    ...
                    if (argTypes[0].isCharType()) {
                        if (!SqlOperator.isCharTypeComparable(argTypes, 0, 1)) {
                            throw EigenbaseResource.instance().newTypeNotComparable(argTypes[0].toString(), argTypes[1].toString());
                        }
        ...
        {{SqlTypeName.Any}, {SqlTypeName.Any}}) {
                    ...
                    SqlCall call)
            {
                if (!checkNoThrowing(call,validator,scope)) {
                    throw EigenbaseResource.instance().newNeedSameTypeParameter(
            ...
            public boolean checkNoThrowing(SqlCall call, SqlValidator validator,
                ...
                RelDataType type1 = validator.deriveType(scope,call.operands[0]);
                RelDataType type2 = validator.deriveType(scope,call.operands[1]);
                RelDataType nullType = validator.typeFactory.createSqlType(SqlTypeName.Null);
    ...
    public static final SqlOperator.AllowedArgInference typeNullableSameSameSame =
        new SqlOperator.AllowedArgInference(new SqlTypeName[][]
            ...
            public boolean checkNoThrowing(SqlCall call, SqlValidator validator,
                ...
                RelDataType type1 = validator.deriveType(scope,call.operands[0]);
                RelDataType type2 = validator.deriveType(scope,call.operands[1]);
                RelDataType type3 = validator.deriveType(scope,call.operands[2]);
                RelDataType nullType = validator.typeFactory.createSqlType(SqlTypeName.Null);
     ...
     public static final SqlOperator.AllowedArgInference typeVarcharLiteral =
         new SqlOperator.AllowedArgInference
                                  ...
                                  SqlNode node, int operandNbr)
             {
                 ...
                 if (!res) return res;
                 if (SqlUtil.isNullLiteral(node, true)) {
                     throw EigenbaseResource.instance().newArgumentMustNotBeNull(call.operator.name);
                 }
                 if (operandNbr == 0) {
     ...
     * @return
     */
    public SqlFunction lookupFunction(String funcName, RelDataType[] argTypes) {
    ...
    public final static RelDataType makeNullableIfOperandsAre(
        ...
        for (int i = 0; i < call.operands.length; ++i) {
            ...
            RelDataType operandType = validator.deriveType(

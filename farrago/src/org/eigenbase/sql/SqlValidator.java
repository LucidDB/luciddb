...
public class SqlValidator
{
    ...
    public SqlValidator(SqlOperatorTable opTab,
            CatalogReader catalogReader, RelDataTypeFactory typeFactory)
    {
    ...
    private RelDataType getTableConstructorRowType(SqlCall values,Scope scope)
    {
        ...
        return typeFactory.createProjectType(
                (RelDataType[]) types.toArray(emptyTypes),
...
//                        " and a mismatched collation charset="+colCharset.name());
    ...
    public RelDataType deriveType(Scope scope, SqlNode operand)
    {
            ...
            for (int i = 0; i < id.names.length; i++) {
                ...
                if (i == 0) {
                    ...
                    if (resolvedScope != null) {
                        ...
                        throw EigenbaseResource.instance().newUnknownIdentifier(name, id.getParserPosition().toString());
                    }
                } else {
        ...
        if (operand instanceof SqlCall) {
            ...
            if (call.operator instanceof SqlCaseOperator) {
                ...
                for(int i=0;i<whenList.size();i++){
                    RelDataType nodeType = deriveType(scope, (SqlNode)whenList.get(i));
                ...
                RelDataType nodeType = deriveType(scope, caseCall.getElseOperand());
                ...
                || (call.operator instanceof SqlRowOperator))
            {
                ...
                if (!(call.operator instanceof SqlJdbcFunctionCall)
                    ...
                    if (function == null) {
                        ...
                        {
                            throw EigenbaseResource.instance().newValidatorUnknownFunction(
                                call.operator.name,
            ...
            if (call.operator instanceof SqlBinaryOperator ||
               ...
               call.operator instanceof SqlPrefixOperator ) {
                ...
                if (call.operator instanceof SqlBinaryOperator) {
                    RelDataType operandType1 = getValidatedNodeType(call.operands[0]);
                    RelDataType operandType2 = getValidatedNodeType(call.operands[1]);
                    ...
                    if (operandType1.isCharType() && operandType2.isCharType()){
                        ...
                        if (!cs1.equals(cs2)) {
                            throw EigenbaseResource.instance().newIncompatibleCharset(
                ...
                //determine coercibility and resulting collation name of unary operator if needed
                    ...
                    RelDataType operandType = getValidatedNodeType(call.operands[0]);
                    if (null==operandType) {
    ...
    private void inferUnknownTypes(
        RelDataType inferredType,Scope scope,SqlNode node)
    {
...
//                    assert(null==node.getParserPosition()) : "todo use pp";
    ...
    private void validateFrom(SqlNode node,RelDataType targetRowType)
    {
        switch (node.getKind().getOrdinal()) {
        case SqlKind.AsORDINAL:
    ...
    private void validateSelect(SqlSelect select,RelDataType targetRowType)
    {
        final SelectScope scope = getScope(select);
        ...
        if (orderList != null) {
            if (!shouldAllowIntermediateOrderBy()) {
                if (select != outermostNode) {
                    throw EigenbaseResource.instance().newInvalidOrderByPos(select.getParserPosition().toString());
                }
            }
        ...
        boolean append)
    {
        ...
        while (iter.hasNext()) {
            ...
            if (iColumn == -1) {
                throw EigenbaseResource.instance().newUnknownTargetColumn(id.getSimple(),id.getParserPosition().toString());
            }
    ...
    private void validateUpdate(SqlUpdate call)
    {
        ...
        RelDataType targetRowType = createTargetRowType(
            table.getRowType(),
            call.getTargetColumnList(),
            true);
    ...
    private void validateValues(SqlCall node,RelDataType targetRowType)
    {
        ...
        if (numOfRows >= 2) {
            ...
            for (int row = 0; row < numOfRows; row++) {
                ...
                if (numOfColumns != thisRow.operands.length) {
                    throw EigenbaseResource.instance().newIncompatibleValueType(node.getParserPosition().toString());
                }
            }
            ...
            for (int col = 0; col < numOfColumns; col++) {
                ...
                final RelDataType type = SqlOperatorTable.useNullableBiggest.
    ...
    private static RelDataType lookupField(final RelDataType rowType, String columnName) {
        final RelDataTypeField [] fields = rowType.getFields();
        for (int i = 0; i < fields.length; i++) {
            RelDataTypeField field = fields[i];
        ...
        public SqlIdentifier fullyQualify(SqlIdentifier identifier)
        {
            ...
            case 2: {
                ...
                if (lookupField(fromScope.getRowType(), columnName) != null) {
                    ...
                    throw EigenbaseResource.instance().newColumnNotFoundInTable(
    ...
    public class SelectScope extends Scope
    {
        ...
        public RelDataType resolveColumn(String name) {
            ...
            for (int i = 0; i < children.size(); i++) {
                ...
                final RelDataType type = lookupField(childScope.getRowType(), name);
                if (type != null) {
                    found++;
                    theType = type;
                }
    ...
    public class IdentifierScope extends Scope
    {
        ...
        public void validate()
        {
            ...
            if (table == null) {
                throw EigenbaseResource.instance().newTableNameNotFound(""+id, id.getParserPosition().toString());
            }
            if (shouldExpandIdentifiers()) {
    ...
    class JoinScope extends Scope
    {
        ...
        public RelDataType getRowType() {
            ...
            final RelDataTypeField [] leftFields = leftScope.getRowType().getFields();
            Scope rightScope = getScope(join.getRight());
            final RelDataTypeField [] rightFields = rightScope.getRowType().getFields();
            final RelDataTypeFactory.FieldInfo fieldInfo = new RelDataTypeFactory.FieldInfo() {

...
bool DoOp_0(
    int iOpCode, int iExType, const char *pExpected,
...
/* --- */
static bool DoOp_1(
    int iTypeCode, int iOpCode, int iExType,
    const char *pExpected, char *pOp0, char *pOp1, char *pResult)
{
    switch (iTypeCode) {
#define DOOP(type) \
    return DoOp_0<type>(iOpCode, iExType, pExpected, pOp0, pOp1, pResult); \
    break;
...
/* --- */
static bool DoOp_2(
    int iType, int iOpCode, int iExType,
    const char *pExpected, char *pOp0, char *pOp1, char *pResult)
{
    try {
        bool bRet = DoOp_1(
...
/* --- */
static bool ReadArgument(
    int iLine, const char *pFile, int iArg, TType &tType,
    const char *ppArgs[], TSuper &t2Read)
{
    ...
    if (*pValue=='&') {
        ...
        fprintf(
            stderr, "Error in argument %d at %s line %d\n", iArg,
            pFile, iLine);
        return false;
    }
    return true;
...
/* --- */
static bool RunTest(
    int iLine, const char *pFile, const char *ppArgs[], int iArgCount)
{
    // op type expected-result op0 [op1]
    if (iArgCount < FIXED_ARGS) {
        fprintf(
            stderr, "Bad number of arguments (%d) at %s line %d\n",
    ...
    if (iOp == -1) {
        fprintf(
            stderr, "No such operation '%s' at %s line %d\n",
    ...
    if (iArgCount < (FIXED_ARGS + iArity)) {
        fprintf(
            stderr,
            "Bad number of arguments (%d) for operation '%s' at %s line %d\n",
    ...
    if (iType == -1) {
        fprintf(
            stderr, "No such type '%s' at %s line %d\n",
    ...
    case '!':
        ...
        break;
    default:
        if (!ReadArgument(iLine, pFile, 2, g_Types[iType], ppArgs, tExpected)) {
            return false;
        }
        iExType = EX_DATA;
    }
    if (iArity > 0
        && !ReadArgument(iLine, pFile, 3, g_Types[iType], ppArgs, tOp0))
    {
        return false;
    }
    if (iArity > 1
        && !ReadArgument(iLine, pFile, 4, g_Types[iType], ppArgs, tOp1))
    {
        return false;
    }
...
{
    assert(pArg);
    while (*pArg && *pArg!=':' && isspace(*pArg)) {
        pArg++;    /* trim leading whitespace */
    }
    if (*pArg == '#') {
        return 0;
    }
...
/* --- */
static bool ProcessALine(
    int iLine, const char *pFile, char *pLine, size_t tLength)
{
    enum { MAX_ARGS = 11 };
    ...
    if (iArgCount == MAX_ARGS) {
        fprintf(
            stderr, "Too many arguments (max=%d) at %s line %d\n",
...
int InputFromStream(const char *pFilename)
{
    ...
    for (;;) {
        ...
        if (!tRead) {
            continue;    /* EOF exactly after BUF_SIZE chars */
        }
...
/* --- return 0 unless (system/format/data) error --- */
int ProcessCppLine(
    int iLine, const char *pInputFileName,
...
{
    // this test is causing a seg. fault, and its not valuable as part
    // of build tests
    return 0;

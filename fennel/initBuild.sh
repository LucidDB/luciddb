...
usage() {
    echo "Usage:  initBuild.sh [--with[out]-farrago] [--with[out]-icu] [--with[out]-optimization] [--with[out]-debug] [--skip-thirdparty] [--with[out]-tests]"
}
...
while [ -n "$1" ]; do
    case $1 in
        ...
        --skip-thirdparty) build_thirdparty=false;;

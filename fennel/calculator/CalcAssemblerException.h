...
public:
    ...
    InvalidTypeException(
        ...
        StandardTypeDescriptorOrdinal expected)
        : FennelExcn(str),
        mInvalidType(type),
        mExpectedType(expected)
    {
        if (msg.length() > 0) {
            msg += ": ";
        }
...
public:
    ...
    InvalidValueException(
        string str, StandardTypeDescriptorOrdinal type, T value)
...
/**
 * A CalcAssemblerException represents a exception generated by the
 * CalcAssembler while assembling a calculator program.
 *
 * The CalcAssemblerException contains a description of the error, and
 * the location in the program where the error occured if available.
...
public:
    ...
    const string getLocationString() const
    {
        ...
        if (mLocValid) {
            ...
            ostr << " (at line:col " << mLoc.first_line
                 << ":" << mLoc.first_column
                 << " to " << mLoc.last_line << ":" << mLoc.last_column
                 << ", characters " << mLoc.first_pos
                 << " to " << mLoc.last_pos << ")";
            locStr = ostr.str();
        } else {

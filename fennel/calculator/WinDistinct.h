...
#ifndef Fennel_WinDistinct_Included
#define Fennel_WinDistinct_Included
...
class WinDistinct : WinDistinctBase
{
...
public:
    ...
    //typedef std::map<STDTYPE, int64_t> WinDistinctData;
    typedef boost::unordered_map<STDTYPE, int64_t> WinDistinctData;
    ...
    {
        // Add the new node to the histogram structure

        if (!node->isNull()) {
            STDTYPE val = node->value();
            typename WinDistinctData::iterator it = currentWindow.find(val);

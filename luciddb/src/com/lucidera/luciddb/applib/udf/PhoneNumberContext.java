...
public class PhoneNumberContext
{
    ...
    /**
     *  parsePhoneNumber()
     *  Parses local representations of local, domestic or international phone 
     *  number.
     *
     *  For example, in San Mateo, California, these numbers are parsed as 
     *  following:
     ...
     *  In HongKong where there is no area code, these numbers are parsed as 
     *  following:
     *     2345-6789           (local to Hong Kong)
     *     852-2345-6789       (local to Hong Kong)
     *     0011-650-513-3870   (international to San Mateo, California)
     *
     *  In addition, an optional extension may exist for all the above, but is 
     *  ignored.
     */
        ...
        throws IllegalArgumentException
    {
        ...
        while (index < length) {
            ...
            if (Character.isDigit(c)) {
                ...
                if ((sections.size() == 1)
                    && (begin > 0) && (phoneNumberStr.charAt(begin-1) == '+'))
                {
                    hasCountryCode = true;
                }
            } else if ((c == 'x') || (c == 'X')) {
        ...
        if (sections.size() == 0) {
            throw new IllegalArgumentException(res.InvalidPhoneNumber.ex());
        }
            ...
            if (entire != null) {
                ...
                for (int i = 0; i < noAreaCodeCountries.length; i++) {
                    if (entire.startsWith(noAreaCodeCountries[i])) {
                        ...
                        pn.localNumber = entire.substring(
        ...
        if (firstSection.startsWith(internationalDialCode)) {
            String remainder = firstSection.substring(
                internationalDialCode.length());
        ...
        if (sections.size() > 1) {
            ...
            if (firstSection.startsWith(domesticDialCode)) {
                pn.areaCode = firstSection.substring(
            ...
            if (firstSection.startsWith(localCountryCode)) {
                firstSection = firstSection.substring(
    ...
    private void handleInternational(PhoneNumber pn, Vector sections, 
        int begin) throws IllegalArgumentException
    {
     ...
     */
    private void validateNANP(PhoneNumber pn) throws IllegalArgumentException
    {
	if (pn.countryCode.equals("1")) {
	    ...
	    } else if ((pn.areaCode.length() != 3)
                || (pn.localNumber.length() != 7))
            {
		throw new IllegalArgumentException(
                    res.InvalidNaPhoneNumber.ex());
            }
	}
    }
	...
	PhoneNumber pn = new PhoneNumber();
        ...
        if ((countryCode == null) || (countryCode.length() == 0)) {
            pn.countryCode = localCountryCode;
	    pn.areaCode = ((areaCode == null) || (areaCode.length() == 0)) 
        ...
        pn.localNumber = ((localNumber == null) || (localNumber.length() == 0))
            ? "" : localNumber;
        pn.extension = ((extension == null) || (extension.length() == 0))
            ? "" : extension;
	...
	for (int i = 0; i < len; i++) {
		...
		throw new IllegalArgumentException(
                    res.PhoneCountryCodeSpecifyDigits.ex());
            }
        }
	...
	for (int i = 0; i < len; i++) {
		...
		throw new IllegalArgumentException(
                    res.PhoneAreaCodeSpecifyDigits.ex());
            }
        }
	...
	for (int i = 0; i < len; i++) {
		...
		throw new IllegalArgumentException(
                    res.PhoneLocalNumberSpecifyDigits.ex());
            }
        }

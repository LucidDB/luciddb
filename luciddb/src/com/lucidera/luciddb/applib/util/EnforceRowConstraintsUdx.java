...
public abstract class EnforceRowConstraintsUdx
{
    ...
    {
        execute(
            inputSet, constraintColumns, resourceJarName, null,
            resultInserter);
    }
        ...
        throws ApplibException
    {
        ...
        resourceJarName = (resourceJarName == null) 
            ? DEFAULT_MESSAGE_CATALOG : resourceJarName;
        try {
            // each row   
            while (inputSet.next()) {
                ...
                for (int i = 0; i < nConstraints; i++) {
                    ...
                    if (errorCode != null) {
                        ...
                        nonCheckColumns = getNonCheckColumnNames(
                            nonCheckColumns,
                            constraintColumns,
                            rsmd,
                            nInput);                        
                        nonCheckColumnValues = getNonCheckColumnValues(
                        ...
                        {
                            deferedException = 
                                (RuntimeException)ApplibResourceObject.get(
                                    ).RowConstraintsFatal.ex(
                                        errorCode, errMsg);
                        } 
                        // filter rows which are reject and fatal
                        keepRow = (keepRow && isWarning(errSev));
                         
                        Object ret = FarragoUdrRuntime.handleRowError(
        ...
        throws ApplibException
    {
        ...
        if (invalidColumns != null) {
            throw ApplibResourceObject.get().InvalidCheckColumns.ex(
                invalidColumns, unfoundColEx);
        }
    }
        ...
        if (doInsert) {
            try {
                ...
                throw ApplibResourceObject.get(
                    ).DatabaseAccessError.ex(Util.getMessages(ex), ex);
            }
        }
    }
    ...
    private static String expandColumns(
        String msg, List<String> columnNames, List<Object> columnValues)
    {
        ...
        throws ApplibException
    {
        ...
        File messageJar =
            new File(pluginDir, jarName+".jar");
        String pathToJar = null;
        try {
            pathToJar = messageJar.getCanonicalPath();
        } catch (java.io.IOException e) {
            throw ApplibResourceObject.get().MessageCatalogLoadFailed.ex(
                jarName, null, null, e.toString());
        }

        // check that file exists
        if (!messageJar.exists()) {
            throw ApplibResourceObject.get().NoSuchMessageCatalogJar.ex(
                jarName, pathToJar);
        }
        ...
        try {
            ...
            rb = 
                ResourceBundle.getBundle(
                    rbBaseName, Locale.getDefault(), urlClassLoader);
        } catch (Throwable ex) {
            throw ApplibResourceObject.get().MessageCatalogLoadFailed.ex(
                jarName, rbBaseName, pathToJar, ex.toString());
        }

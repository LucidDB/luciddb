...
create or replace view dba_auth_ids as
  select
    ...
    cast("visibility" as varchar(128)) as visibility,
    ...
    cast("namespace" as varchar(128)) as namespace
...
create or replace view dba_users as
  select
    ...
    auth.class_name,
    auth.visibility,
    auth.creation_timestamp,
    auth.mof_id,
    auth.lineage_id,
    auth.namespace
  from
    sys_root.dba_auth_ids as auth
  inner join
    sys_fem."Security"."User" users
  on
    users."mofId" = auth.mof_id;

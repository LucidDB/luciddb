-- Broadbase query set with PGE prefixes for tables and timing on.
-- This script tests the timing of queries in BB by executing 20 queries
-- and reporting the elapsed time for each query.

-- Written by Carolyn Chau
-- Last Modified: 5/27/97

-- Execute this script by typing
-- idba user=PGE passwd=PGE dsource=BB_AUTO_DS_AAHNOLD server=AAHNOLD time=ON echo=ON file=i:\clients\broadbase\prebeta\queries\bball.idba plan=BOTH > bball.log
--
...
from pge.revn_dtl_rand
group by cust_serv_acct_key having rank(topcust_amt) <= 10
;

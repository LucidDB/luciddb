...
select sum(PRICE) from SALES group by PRODID having PRODID < 10010 order by PRODID;

select count(PRICE) from SALES group by PRODID having PRODID > 10010 and PRODID/2<5007 order by PRODID;
select count(*) from SALES group by PRODID having PRODID > 10010 and PRODID/2<5007 order by PRODID;


select floor(avg(PRICE)) from SALES group by PRODID having PRODID in (10005, 10007, 10009) order by PRODID;
...
select count(PRICE) from SALES where PRICE+0 < 5.00 group by PRODID order by PRODID;
select count(*) from SALES where PRICE+0 < 5.00 group by PRODID order by PRODID;

select floor(avg(PRICE)) from SALES where EMPNO < 100 or EMPNO > 107 group by PRODID order by PRODID;

select min(PRICE), max(PRICE) from SALES where EMPNO*2 between 204 and 212 group by PRODID order by PRODID;

...
public class OJValidator extends VariableBinder
{
    ...
    public Expression evaluateUp(FieldAccess p) throws ParseTreeException
    {
        ...
        if (!fieldExists(p.getReference(),name,env)) {
            OJClass reftype = Toolbox.getType(env,p.getReference());
            Signature signature = new Signature(name);
            throw Util.newInternal(
                "no " + signature + " is accessible in type '" + reftype + "'");
        }
        return super.evaluateUp(p);
    }
    ...
    public Expression evaluateUp(MethodCall p) throws ParseTreeException
    {
        ...
        for (int i = 0; i < argTypes.length; i++) {
            argTypes[i] = Toolbox.getType(env,args.get(i));
        }
        String name = p.getName();
    ...
    public TypeName evaluateUp(TypeName p) throws ParseTreeException
    {
        ...
        OJClass clazz = env.lookupClass(qname,p.getDimension());
        if (clazz == null) {
